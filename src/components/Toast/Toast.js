...
const Toast = ({ title, message, type = 'info', duration = 3000 }) => {
  ...
  const iconMap = {
    success: { icon: faCircleCheck, color: 'text-green-500', border: 'border-green-500' },
    info: { icon: faInfoCircle, color: 'text-blue-500', border: 'border-blue-500' },
    warning: { icon: faExclamationCircle, color: 'text-yellow-500', border: 'border-yellow-500' },
    error: { icon: faTimesCircle, color: 'text-red-500', border: 'border-red-500' },
  };
